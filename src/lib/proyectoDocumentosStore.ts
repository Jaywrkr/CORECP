...
export async function guardarArchivoProyecto(
  proyectoId: string,
  tipo: string,
  file: File,
): Promise<{ url: string; nombre: string }> {
  const tipoSlug = sanitizeSegment(tipo);
  const pathname = `proyectos/documentos/${proyectoId}/${tipoSlug}-${sanitizeFilename(file.name)}`;
...
export async function eliminarArchivoProyecto(url: string): Promise<void> {
  try {
    await del(url);
  } catch {
